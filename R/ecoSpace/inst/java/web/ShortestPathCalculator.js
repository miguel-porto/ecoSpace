...
var ShortestPathCalculator = function(nodes, paths) {
	...
	if(!nodes.length || nodes.length > maxNodes || nodes.length < minNodes) return;
...
ShortestPathCalculator.prototype.findRoute = function(source, target) {
	if(!ShortestPathCalculator.isInteger(source) || !ShortestPathCalculator.isInteger(target)) return;
...
//		var d = parseInt(this.paths[i].distance);
		var d=1;	// distance here is always 1
		if(!d) {this.stop=true;}
				...
				.attr("class", function(d) {
					if(that.result.path !== null) {
						for (var i = 0; i < that.result.path.length; i++) {
							if ((that.result.path[i].source.index === d.source.index && that.result.path[i].target.index === d.target.index)
							 || (that.result.path[i].source.index === d.target.index && that.result.path[i].target.index === d.source.index))
								return 'link bold';
						}
					}
					return 'link';
...
ShortestPathCalculator.prototype.formatResult = function() {
	...
	for(var i=0; i<this.result.path.length; i++) {
		var sourceNodeIndex = this.result.path[i].source.index;
		var targetNodeIndex = this.result.path[i].target.index;
